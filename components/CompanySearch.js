...
export function detectIndustryFromSic(sic, sicDescription) {
...
export default function CompanySearch({ onCompanyLoaded, onStatusChange, onYearSelectorReady }) {
  ...
  const handleSelect = useCallback(async (company) => {
    ...
    try {
      ...
      // Pass year selector data to parent
      if (onYearSelectorReady && availableYears.length > 1) {
        onYearSelectorReady({ company, filingsData, availableYears, selectedYear });
      }
    ...
    setLoading(null);
  }, [onCompanyLoaded, onStatusChange, onYearSelectorReady]);
